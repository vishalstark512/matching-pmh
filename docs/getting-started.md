...
4. **Train** with `PMHLoss` on top of your task loss at a chosen layer $h=\phi(x)$.
5. **Evaluate** matched vs **wrong-W** and **isotropic** (and **signal-W** when you have a clear signal subspace).

**Practical wiring (CNN, HF Trainer, GNN, sklearn):** [ARCHITECTURES.md](ARCHITECTURES.md)  
**Math and recipe:** [THEORY.md](THEORY.md)
...
`h_source`, `h_target` are `[N, d]` tensors from **your** encoder on source/target data (frozen or warm). See [ARCHITECTURES.md](ARCHITECTURES.md).

CLI: `pmh-train estimate --config job.json` — [CLI](cli.md).
...
h = backbone(x)   # same layer / dim as Phase A
