...
**Fidelity detail (default vs calibrator):** see rows below · **CI goldens:** `tests/test_subtype_fidelity.py`
...
|-------|-------|----------------------------------|-----------|---------------------------|---------------|-------------------|
| **T1** Office-31 | D1 | Class-aligned cross-domain SVD + mean shift | **Mode B** projection | rank 32, pool 200, test 250 | [PAPER_ALIGNMENT](PAPER_ALIGNMENT.md) (T1) · `compare_arms_sklearn`, `paper_protocol=True` | matched, wrong-W⊥W, D4-iso; CORAL baseline |
...
| **T4A/B** DA vision | D4 | Per-layer domain Gram | Mode A multilayer | gram_rank 64, layer hooks | [walkthrough 01](walkthroughs/01-pytorch-domain-d4.md) · `nuisance="domain_shift"`, `MultiLayerPMHLoss` | E1 pixel iso vs multiscale |
